...
class SimpleVizMode extends BaseVizMode {
    ...
    drawLinks() {
        ...
        let p1 = this.node_reps[1].getShapeMesh().position;
        let cone_start = create_arrow("arrow_start", this.node_reps[0], p1, 0.125, MATERIALS[GREEN]);

        let node_count = this.node_reps.length;
        p1 = this.node_reps[node_count-2].getShapeMesh().position;
        let cone_end = create_arrow("arrow_end", this.node_reps[node_count-1], p1, 0.125, MATERIALS[RED]);
        cone_end.rotation.x += Math.PI;
    } // drawLinks()
